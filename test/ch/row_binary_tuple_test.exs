defmodule Ch.RowBinaryTupleTest do
  ...
  test "named Tuple params and RowBinary values round-trip through ClickHouse", %{pool: pool} do
    type = "Tuple(name String, count UInt8, active Bool)"
    value = {"example", 42, true}

    assert Ch.query!(pool, "SELECT {value:#{type}}", %{"value" => value}).rows == [[value]]

    Ch.Test.query("CREATE TABLE row_binary_named_tuple(id UInt64, value #{type}) ENGINE Memory")
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_named_tuple") end)

    rows = [[0, {"zero", 0, false}], [18_446_744_073_709_551_615, value]]
    rowbinary = RowBinary.encode_rows(rows, ["UInt64", type])

    Ch.query!(pool, "INSERT INTO row_binary_named_tuple FORMAT RowBinary", rowbinary,
      encode: false
    )

    assert Ch.query!(pool, "SELECT * FROM row_binary_named_tuple ORDER BY id").rows == rows
  end
  ...
  property "RowBinary Tuple inserts round-trip through ClickHouse", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_tuple_property") end)
    ...
    check all rows <- rowbinary_tuple_rows(), max_runs: 25 do
      ...
      Ch.query!(pool, "INSERT INTO row_binary_tuple_property FORMAT RowBinary", rowbinary,
        encode: false
      )
  ...
  test "RowBinary Tuple inserts cover nested containers and nil defaults", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_tuple_representative") end)
    ...
    Ch.query!(pool, "INSERT INTO row_binary_tuple_representative FORMAT RowBinary", rowbinary,
      encode: false
    )
