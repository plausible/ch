defmodule Ch.RowBinaryTupleTest do
  ...
  test "named Tuple params and RowBinary values round-trip through ClickHouse", %{pool: pool} do
    ...
    Help.query!("CREATE TABLE row_binary_named_tuple(id UInt64, value #{type}) ENGINE Memory")
    on_exit(fn -> Help.query!("DROP TABLE row_binary_named_tuple") end)

    rows = [[0, {"zero", 0, false}], [18_446_744_073_709_551_615, value]]
    rowbinary = RowBinary.encode_rows(rows, ["UInt64", type])
    Ch.query!(pool, ["INSERT INTO row_binary_named_tuple FORMAT RowBinary\n" | rowbinary])
