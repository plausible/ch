defmodule Ch.RowBinaryGeoTest do
  ...
  property "RowBinary Point inserts round-trip through ClickHouse", %{pool: pool} do
    Ch.Test.query("CREATE TABLE row_binary_geo_point_property(id UInt64, p Point) ENGINE Memory")
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_geo_point_property") end)
    ...
    check all rows <- point_rows(), max_runs: 25 do
      ...
      Ch.query!(pool, "INSERT INTO row_binary_geo_point_property FORMAT RowBinary", rowbinary,
        encode: false
      )
  ...
  test "RowBinary inserts cover Ring, Polygon, and MultiPolygon aliases", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_geo_values") end)
    ...
    Ch.query!(pool, "INSERT INTO row_binary_geo_values FORMAT RowBinary", rowbinary,
      encode: false
    )
