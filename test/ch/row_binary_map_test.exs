defmodule Ch.RowBinaryMapTest do
  ...
  property "RowBinary Map inserts round-trip through ClickHouse", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_map_property") end)
    ...
    check all rows <- rowbinary_map_rows(), max_runs: 25 do
      ...
      Ch.query!(pool, "INSERT INTO row_binary_map_property FORMAT RowBinary", rowbinary,
        encode: false
      )
  ...
  test "RowBinary Map inserts cover nested and nullable values", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_map_representative") end)
    ...
    Ch.query!(pool, "INSERT INTO row_binary_map_representative FORMAT RowBinary", rowbinary,
      encode: false
    )
