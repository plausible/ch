defmodule Ch.RowBinaryNetworkTest do
  ...
  property "RowBinary network inserts round-trip through ClickHouse", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_network_property") end)
    ...
    check all rows <- rowbinary_network_rows() do
      ...
      Ch.query!(pool, "INSERT INTO row_binary_network_property FORMAT RowBinary", rowbinary,
        encode: false
      )
  ...
  test "RowBinary inserts cover nullable, arrays, tuples, and defaults", %{pool: pool} do
    ...
    on_exit(fn -> Ch.Test.query("DROP TABLE row_binary_network_representative") end)
